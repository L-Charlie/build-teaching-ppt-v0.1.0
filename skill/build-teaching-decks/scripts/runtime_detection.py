...
def detect_executor(explicit: str | None = None) -> dict[str, object]:
    ...
    override = os.environ.get("TEACHING_DECK_EXECUTOR")
    if override:
        selected = normalize_executor(override)
        if selected == "auto":
            raise ValueError("TEACHING_DECK_EXECUTOR cannot be set to auto")
        return {
            "name": selected,
            "method": "environment-override",
            "evidence": ["TEACHING_DECK_EXECUTOR"],
        }
...
def host_presentations_requested(executor: str) -> bool:
    configured = os.environ.get("TEACHING_DECK_HOST_PRESENTATIONS", "").strip().lower()
