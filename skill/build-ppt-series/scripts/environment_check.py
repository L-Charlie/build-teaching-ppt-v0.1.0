#!/usr/bin/env python3
"""Report PPT-series workflow capabilities without changing the environment."""
...
IMAGE_BACKENDS = ("available", "unavailable", "unknown")


def build_report(executor: str = "auto", image_backend: str = "unknown") -> dict[str, object]:
    executables = {
    ...
    return {
        ...
        "capabilities": {
            ...
            "image_generation": image_backend == "available",
        },
        "image_backend": {
            "status": image_backend,
            "source": "agent-declaration",
            "note": "Host image tools cannot be discovered reliably by this local script.",
        },
    }
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        choices=EXECUTORS,
        default="auto",
        help="Override detection; PPT_SERIES_EXECUTOR and legacy TEACHING_DECK_EXECUTOR are supported",
    )
    parser.add_argument(
        "--image-backend",
        choices=IMAGE_BACKENDS,
        default="unknown",
        help="Declare host image-generation capability after inspecting available tools",
    )
    parser.add_argument(
        "--require",
        choices=(
    ...
    report = build_report(args.executor, args.image_backend)
    if args.json:
        ...
        print(f"Image generation: {report['image_backend']['status']} (agent declaration)")
    if args.require:
